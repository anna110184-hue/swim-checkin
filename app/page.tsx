...
export default async function HomePage() {
  ...
  function getSetting(key: string, fallback = "true") {
    return ((settings ?? []) as any[]).find((s) => s.key === key)?.value ?? fallback;
  }

  const showSat = getSetting("show_saturday") === "true";
  const showSun = getSetting("show_sunday") === "true";
  ...
  function buildCardData(student: any) {
    ...
    const paidLessons: number | null = student.sessions?.[0]?.paid_lessons ?? null;
    ...
    return { ...student, attended, total, paidLessons, checkedInToday, substituteToday, paymentClaimedToday, attendanceDates, missedDates };
  }
  ...
  const defaultTab: "sat" | "sun" = todayDow === "sun" && showSun ? "sun" : showSat ? "sat" : showSun ? "sun" : "sat";
